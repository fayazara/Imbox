...
class Mailjs {
    ...
    on(event, callback) {
        const allowedEvents = ["seen", "delete", "arrive", "error", "open", "ready"];
        ...
        if (!this.listener) {
            ...
            this.listener = new EventSourcePolyfill(`${this.baseMercure}?topic=/accounts/${this.id}`, options);

            this.listener.onmessage = (event) => {
                const data = JSON.parse(event.data);
                // Checking the event type
                if (data.seen && this.events.seen) {
                    this.events.seen(data);
                } else if (data.isDeleted && this.events.delete) {
                    this.events.delete(data);
                } else if (this.events.arrive) {
